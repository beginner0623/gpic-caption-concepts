...
1. GPIC 결과는 rule의 정답이 아니라 candidate generator로만 쓴다.
2. 사람 audit은 필수 단계로 두지 않는다. 모든 승격/거절은 automatic rubric audit, source 근거, confidence, regression 결과로 결정한다.
3. frozen lexicon에 넣는 항목은 source, decision, confidence, reason, audit_version을 남긴다.
4. 외부 source 기반 후보와 GPIC failure 기반 후보를 분리해서 저장한다.
5. 버전은 freeze한다. 예를 들어 `v1` 결과는 그대로 두고, 새 후보는 `v2_candidate`로 쌓은 뒤 automatic audit 후 승격한다.
6. caption을 100M개 처리해야 하므로, 최종 규칙은 lightweight lookup/rule이어야 한다. Maverick/LLM은 후보 생성이나 automatic audit 단계에만 쓰는 것을 기본으로 한다.
...
| `reports/feedback_dashboard_<sample>.md` | 자동 요약된 failure/pattern dashboard |
| `resources/lexicons/*_candidate.tsv` | 승격 전 후보 |
| `resources/lexicons/*_audit_flags.tsv` | 자동 reject/maybe와 이유 |
| `resources/lexicons/*_auto_frozen_core_vN.tsv` | 실제 parser에 적용하는 자동 audit 기반 frozen whitelist |
| `docs/caption_to_concept_decision_log.md` | 결정과 결과 시계열 기록 |


## Human Audit Policy

사람 audit은 필수 단계로 두지 않는다. 목표는 100M captions를 3일 안에 처리할 수 있는 자동 pipeline이므로, 사람이 후보를 직접 고르거나 최종 승인하는 구조를 pipeline dependency로 만들지 않는다.

| 구분 | 정책 |
|---|---|
| candidate 수집 | 완전 자동 |
| source 교차 검증 | 완전 자동 |
| Codex/LLM rubric audit | 완전 자동, JSON/TSV log 저장 |
| confidence 산정 | 완전 자동 |
| freeze 여부 | threshold + regression test로 자동 결정 |
| 사람 검토 | optional sanity check, pipeline blocking 조건 아님 |

따라서 사람이 보지 않은 lexicon에는 `clean`이나 `gold`라는 이름을 쓰지 않는다. 대신 다음 명칭을 쓴다.

| 이름 | 의미 |
|---|---|
| `candidate` | 아직 적용하지 않는 후보 |
| `model_audited` | Codex/LLM rubric audit을 통과한 후보 |
| `auto_frozen` | regression test를 통과해 parser에 적용하는 frozen whitelist |
| `audit_flags` | 자동 reject/maybe와 그 이유 |

최종 논문/보고서 표현도 `manually curated`가 아니라 `automatically audited with a fixed rubric`로 쓴다.
...
Maverick을 rule 자체로 쓰지는 않는다. 1M caption에 대해 coreference 후보를 만들고, 다음 compact 정보만 저장한다.
...
| external plausibility | WordNet/OEWN 또는 frozen project ontology에서 generic parent로 설명 가능 |
...
auto_frozen 승격은 GPIC 빈도만으로 하지 않는다. COCO/LVIS/Open Images/VG object labels, WordNet/OEWN noun synset, Wiktionary/Kaikki noun entry 같은 외부 근거를 같이 기록한다.
...
| bucket | 수집 기준 | 승격 방식 |
|---|---|---|
| object synonym | `cab -> taxi`, `puppy -> dog` | external source + project alias |
...
| attribute synonym/type | raw_attribute high-frequency | typed visual attribute lexicon만 승격 |
...
## 6. 현재 진행 상태와 다음 실행 순서

2026-06-29 기준으로 다음까지 완료했다.

1. 10k train sample을 실행해 feedback 후보를 수집했다.
2. 사람이 audit하지 않는 automatic audit/freeze 정책을 문서화했다.
3. `auto_frozen_v1` lexicon을 추가했다.
4. Stage 9 object synonym/parent, action synonym/parent, attribute type 일부를 보수적으로 확장했다.
5. Stage 8에서 `’s`/`'s`류 apostrophe 조각 action noise만 차단했다.
6. 10k와 기존 100개 두 세트에서 regression을 다시 돌렸다.

`auto_frozen_v1` 후 10k 주요 변화:
...
- parent long-tail은 아직 크다.
- `be`, `come`, `let`, `feel` 같은 wide predicate는 별도 정책이 필요하다.
- self-edge repair는 lexicon 확장으로 해결되지 않는다.
- object MWE, preposition MWE, phrasal action은 후보 수집 이후 freeze 단계가 더 필요하다.

다음 실행 순서는 아래처럼 갱신한다.

1. 남은 `entity_parent_none` high-frequency 후보를 v2 parent queue로 만든다.
2. action fallback을 `countable_action`, `visual_state`, `text_document_predicate`, `noise_or_auxiliary`, `exclude`로 자동 분류한다.
3. `be` copular/state 정책을 정한다. 바로 제거하지 말고, 먼저 count impact를 따로 측정한다.
4. self-edge repair analyzer를 만든다. 바로 repair하지 말고 alternative antecedent 후보와 confidence를 기록한다.
5. object MWE / preposition MWE / phrasal action 후보를 각각 별도 `auto_frozen_v2` 후보로 분리한다.
6. 같은 10k + 기존 100개 두 세트에서 before/after regression을 반복한다.
7. Maverick은 1M 전체 전에 작은 pilot로 throughput과 output schema를 확정한다.
8. Maverick 1M은 full graph를 저장하지 말고 generic anaphoric/coref candidate만 압축 저장한다.

이 방식이면 GPIC를 돌릴수록 rule이 누적되지만, 각 rule이 어디서 왔고 왜 들어갔는지 추적 가능하다. 사람 검토는 pipeline 필수 단계가 아니라, 논문 작성이나 최종 릴리스 전에 참고할 수 있는 선택적 sanity check로만 둔다.
