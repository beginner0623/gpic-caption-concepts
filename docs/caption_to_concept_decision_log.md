...
별도 이슈로 다루는 것이 맞다.
```
## 2026-06-27 - P0 mention id collision 수정

### 문제

`generic_anaphoric` reference를 추가한 뒤 일부 object mention을 alias merge로 제거하면서 mention id가 재사용되는 문제가 발생했다.
원인은 `add_mention()`이 `len(self.mentions)` 기반으로 id를 발급했기 때문이다.

대표 증상:

```text
case 38:
  m16 reference The structure
  m16 action surrounded

case 80:
  m16 reference The object
  m16 action rests
```

이 상태에서는 edge의 `source=m16`이 reference인지 action인지 구분되지 않으므로 graph consumer 입장에서는 치명적인 serialization 오류다.

### 수정

`RawConceptExtractor`에 monotonic mention id counter를 추가했다.
...
이제 mention list에서 alias/drop으로 항목이 제거되어도 id는 절대 재사용하지 않는다.

### 검증

100개 eval shard 재생성 후 전체 JSONL에서 mention id duplicate를 검사했다.

```text
duplicate_records: 0
```

문제가 보고된 케이스에서도 reference/action id가 분리됨을 확인했다.

| case | reference id | action id 예시 |
|---:|---|---|
| 38 | `m16` The structure | `m17` surrounded |
| 49 | `m15` The individual | `m16` wears |
