...
for row in caption_jsonl:
    ...
    if mask_quotes:
        parse_caption, quote_mentions = mask_quoted_text(caption)
    else:
...
```text
A screen shows the quoted text.
```
...
```text
quote 내부 title/text가 문장 구조를 깨는 것을 막기 위해서
```

8단계 raw extraction에서는 `quoted text`가 object/attribute로 남을 수 있지만, 원본 quote 내용은 별도 metadata로 복원 가능하다.
