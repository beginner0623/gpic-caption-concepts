...
def write_report(path: Path, inputs: list[Path], audit: Audit, top_n: int) -> None:
    ...
    lines.append("")
    lines.append("이 문서는 Stage 9 canonical v2 결과에서 다음 확장/수정 후보를 자동으로 긁은 audit report다. 여기 나온 항목은 모두 오류 확정이 아니라, 사람이 우선 검토할 후보로 봐야 한다.")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
    for p in inputs:
    ...
    lines.append(f"2. `action_parent_fallback`은 {action_fallback_count}개다. 이 중 일부는 parent만 추가하면 되고, 일부는 action으로 세면 안 되는 predicate인지 재검토해야 한다.")
    ...
    lines.append("5. reference-like entity는 실패 확정이 아니다. `one/other/another`류 instance split이 제대로 됐는지 보는 검토 큐로 쓰면 된다.")
    lines.append("")
    ...
    lines.append("")
    lines.append("아래는 parent가 있더라도 countable visual action으로 세는 정책을 재검토할 만한 predicate다.")
    lines.append("")
